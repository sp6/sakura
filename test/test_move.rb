...
class TC_Shogi < Test::Unit::TestCase
  ...
  def test_move
    ...
    @k.teban = :sente
    te = @k.generate_legal_moves
    assert_equal(9, te.size)
...
=end
    ...
    @k.hand = {
      sente: {
        fu: 0, kyosha: 0, keima: 0, gin: 0, kin: 1, kaku: 0, hisha: 1
      },
      gote: {
        fu: 5, kyosha: 0, keima: 1, gin: 1, kin: 1, kaku: 0, hisha: 0
      }
    }
    te = @k.generate_legal_moves
...
EOS
    te = Te.new(:sente, Pos.new(7, 7), Pos.new(7, 6), Koma::SFU)
    @k.move te
   kifu = <<EOS
P1-KY-KE-GI-KI-OU-KI-GI-KE-KY
P2 * -HI *  *  *  *  * -KA * 
